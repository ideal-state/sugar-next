...
public abstract class Reflection {
    ...
    public static <A extends Annotation> A annotation(@NotNull Class<A> annotationType, Map<String, Object> mappings) {
        ...
        Validation.is(Annotation.class.isAssignableFrom(annotationType), "annotationType must be an Annotation.");
        Validation.notNull(mappings, "mappings must not be null");

        return (A) Proxy.newProxyInstance(
                annotationType.getClassLoader(), new Class[] {annotationType}, new InternalAnnotationHandler(mappings));
    }
}
