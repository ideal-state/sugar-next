...
package team.idealstate.sugar.next.io;
...
public abstract class IOUtils {
    ...
    public static byte[] readAllBytes(@NotNull InputStream inputStream) throws IOException {
        ...
        return functional(new ByteArrayOutputStream()).use(byte[].class, output -> {
            try {
    ...
    public static long transferTo(@NotNull InputStream inputStream, @NotNull OutputStream outputStream) {
        Validation.notNull(inputStream, "inputStream must not be null.");
        Validation.notNull(outputStream, "outputStream must not be null.");
        return functional(inputStream)
                .use(Long.class, input -> functional(outputStream).use(Long.class, output -> {
                    long transferred = 0;
