...
package team.idealstate.minecraft.next.common.context.boot.mybatis.factory;
...
import team.idealstate.minecraft.next.common.context.boot.mybatis.MyBatis;
...
import team.idealstate.minecraft.next.common.database.TransactionManager;
import team.idealstate.minecraft.next.common.database.annotation.Transaction;
...
public class TransactionComponentBeanFactory extends ComponentBeanFactory {
    ...
    protected <T> T doProxy(
        ...
        Class<?> proxyType =
                functional(
                        ...
                        .use(
                                Class.class,
                                unloaded -> unloaded.load(context.getClassLoader()).getLoaded());
        try {
    ...
    @NotNull static TransactionManager getTransactionManager(@NotNull Context context) {
        List<Bean<TransactionManager>> beans = context.getBeans(TransactionManager.class);
        Validation.is(
                beans.size() == 1,
                "there are multiple transaction managers in the current context.");
        Bean<TransactionManager> bean = beans.get(0);
        Validation.is(
                MyBatis.class.isAssignableFrom(bean.getMarked()),
                "transaction manager must be MyBatis.");
    ...
    protected static final class TransactionInterceptor {
        ...
        @NonNull private final Map<String, Transaction> transactions;
