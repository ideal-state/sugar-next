...
public interface CommandLine {
    ...
    @NotNull static String validateName(@NotNull String name) {
            ...
            if (header) {
                if (!(('A' <= c && c <= 'Z') || ('a' <= c && c <= 'z'))) {
                    throw new IllegalArgumentException(
                            parameterName + " must start with a English letter.");
                }
                header = false;
            }
            if (!(('A' <= c && c <= 'Z') || ('a' <= c && c <= 'z') || ('0' <= c && c <= '9'))) {
                throw new IllegalArgumentException(
                        parameterName + " must contain only English letters and numbers.");
            }
        }
        return name;
    }
    ...
    @NotNull CommandResult execute(@NotNull CommandContext context, @NotNull String... arguments)
            throws CommandException;

    @NotNull List<String> complete(@NotNull CommandContext context, @NotNull String... arguments)
            throws CommandException;
}
