...
package team.idealstate.minecraft.next.common.hex;
...
public abstract class HexUtils {
    ...
    @NotNull public static byte[] hexToBinary(@NotNull String hex) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            binary[i] =
                    (byte)
                            ((HEX_DIGIT_TABLE.get(hex.charAt(j)) << 4)
                                    + HEX_DIGIT_TABLE.get(hex.charAt(j + 1)));
        }
        return binary;
    }

    @NotNull public static String binaryToHex(@NotNull byte[] binary) {
